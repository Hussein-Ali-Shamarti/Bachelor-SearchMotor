...
# Function to insert or update conference info into the Conferences table
def insert_or_update_conference_info(conference_data, session):
    ...
    if existing_conference:
        ...
        existing_conference.articles = ",".join(conference_data["Articles"])
        session.commit()  # Commit the changes
        print(f"Conference '{conference_title}' updated with new articles.")
    else:
        # Insert new conference
        new_conference = Conference(
            name=conference_data["Conference Name"],
            articles=",".join(conference_data["Articles"])
        )
        session.add(new_conference)
        session.commit()
        print(f"Inserted new conference: {conference_title} with ID: {new_conference.id}")


# Function to insert or update article info into the Articles table
def insert_or_update_article_info(article_data, session):
    print(f"Inserting/updating article: {article_data['title']}")  # Debugging
    ...
    try:
        ...
        if existing_article:
            ...
            existing_article.author = ", ".join(article_data.get("author", []))
            existing_article.publication_date = article_data.get("publication_date", "")
            existing_article.pdf_url = article_data.get("pdf_url", "")
            existing_article.pdf_texts = article_data.get("pdf_texts", "")
            existing_article.authors = ", ".join(article_data.get("Authors", []))
            existing_article.keywords = article_data.get("Keywords", "")
            existing_article.abstract = article_data.get("Abstract", "")
            existing_article.location = article_data.get("Location", "")
            existing_article.start_date = article_data.get("start_date", "Unknown")
            existing_article.end_date = article_data.get("end_date", "Unknown")
            existing_article.conference_id = conference_id
            session.commit()
            print(f"Article '{article_data['title']}' updated with Conference ID: {existing_article.conference_id}")
        else:
            # Insert new article
            new_article = Article(
        ...
        print(f"[ERROR] Failed to insert/update article '{article_data['title']}': {e}")
        import traceback
        ...
        for file_article in files_article:
            if file_article.endswith(".txt"):
                ...
                if article_data:
                    ...
                    insert_or_update_article_info(article_data, session)  # Insert or update the article
        ...
        for file_name in files:
            if file_name.endswith(".txt"):
                ...
                if conference_data:
                    ...
                    insert_or_update_conference_info(conference_data, session)  # Insert or update the conference
...
def main():
    ...
    # Define conference and articles folders
    conference_folder = os.path.join(source_folder, "conference")
    articles_folder = os.path.join(source_folder, "articles")
